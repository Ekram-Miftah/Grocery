...
const ProductPage = () => {
  ...
  useEffect(() => {
    ...
    setLocalQuantity(1);
    window.scrollTo(0, 0);
    const found = dummyProducts.find((p) => String(p._id) === String(id));
  ...
  const handleAddToCart = () => {
    if (inCart) {
      updateQuantity(product._id, displayQuantity);
    } else {
...
transition-colors"
          ...
          <span>/</span>
          <span
            className="text-app-green font-medium truncate max-w-[200px]
"
          >
            {product.name}
          </span>
...
hover:text-app-green transition-colors"
  ...
  bg-app-orange text-white rounded-full"
            ...
            <div className="p-6 md:p-10 flex flex-col justify-center">
              <span
                className="text-xs font-medium text-app-text-light
  tracking-wider mb-2 capitalize"
              >
                {categoryLabel}
              </span>
              <h1
  ...
  text-app-green"
    ...
    line-through"
              ...
              {/* Description */}
              <p
                className="text-sm text-app-text-light leading-relaxed
mb-6"
              >
                {product.description}
              </p>
